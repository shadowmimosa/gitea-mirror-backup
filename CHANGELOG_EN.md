...
- **Delete reports**: admins can delete reports; protected reports require `force=true`
- **Delete repository backup**: remove entire local `owner/repo` backup directory
- **Source repo status**: `source_exists` flags backups whose Gitea source repo was removed
...
- **Snapshot list filters**
  - API supports `is_protected` and `repository_search` query parameters
  - Snapshot list page adds repo search, protection status filter, and protected-only shortcut
  - Protected snapshots sorted to the top by default

- **Semi-automatic restore wizard**
  - New `POST /api/restore/preview` generates copy-paste restore commands
  - Repository detail page adds restore wizard; Docker deployments get `docker compose run --entrypoint bash` commands
